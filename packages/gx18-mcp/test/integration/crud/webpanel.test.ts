...
describe.skipIf(!SPIKE_AVAILABLE)('CRUD: webpanel', () => {
  let createdName: string;

  beforeAll(() => {
    createdName = testName('Wbp');
  });

  afterAll(async () => {
    if (createdName) await cleanup(createdName, 'webpanel');
  });

  it('create — cria com events e rules', async () => {
